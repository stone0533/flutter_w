...
class WFormBorderState extends State<WFormBorder> {
  ...
  WFormBorderDecoration? _getDecoration() {
    // 检查状态是否变化
    final isStateChanged = _cachedIsTransparentBackground != widget.isTransparentBackground ||
...
class WFormBorderConfig {
  ...
  /// 标签配置回调函数 setter
  set configForTagCallback(WFormBorderConfig? Function(String tag, WFormBorderConfig currentConfig) value) {
...
extension WFormBorderDecorationExtension on WFormBorderDecoration {
  ...
  WFormBorderDecoration withUniformBorder(double width, Color color) {
    return copyWith(border: Border.all(width: width, color: color));
  }
